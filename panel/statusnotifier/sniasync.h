...
struct call_signature
    : public remove_class_type<typename call_sig_helper<L>::type>
{
};
...
class SniAsync : public QObject
{
    Q_OBJECT
public:
    SniAsync(const QString & service, const QString & path,
    ...
    {
        static_assert(
            is_valid_signature<typename call_signature<F>::type>::value,
                ...
                [this, finished, name](QDBusPendingCallWatcher * call) {
                    ...
                    if (reply.isError())
                        qDebug().noquote().nospace()
                            << "Error on DBus request(" << mSni.service() << ','
                            << mSni.path() << "): " << reply.error();
                    finished(
                        qdbus_cast<typename std::function<
